...
- `app/meal_plan_common.py` — общие функции рациона (даты, цели, распределение по приёмам пищи).
...
- `PRODUCTS_DB_PATH` — абсолютный путь к SQLite базе продуктов.
  На Windows допускается формат `/data/...` (будет интерпретирован как `C:\\data\\...`).
...
- `PLANS_PATH` больше не используется: тарифы хранятся в SQLite базе приложения и редактируются в админке.
...
Как открыть:
- В браузере: `PUBLIC_APP_URL` + `/admin`.
- Из Telegram: в чате с ботом отправить команду `/<TELEGRAM_HIDDEN_ADMIN_COMMAND>` и нажать кнопку “Открыть админ-панель”.

Что доступно:
- Продукты: добавление/редактирование/удаление.
- Группы продуктов: добавление/удаление/переименования.
- Нормы и пробный период: нормы (вода/сон/клетчатка) и длительность пробного периода.
- Тарифы: создание и редактирование тарифов (цена, старая цена, срок).
- Пользователи: список и карточка пользователя.
- Мониторинг: страница сводки (для проверки работы и базовой статистики).
