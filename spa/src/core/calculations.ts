...
export interface CalculationsDependencies {
    debug?: boolean;
    normalizeLocalDate?: (value: string) => string | null;
    logger?: Pick<Console, 'log'>;
}

const getLogger = (deps?: CalculationsDependencies): Pick<Console, 'log'> => deps?.logger || console;
const isDebugEnabled = (deps?: CalculationsDependencies): boolean => deps?.debug === true;

function logForecastDebug(debugData, deps: CalculationsDependencies = {}) {
    if (isDebugEnabled(deps)) {
        getLogger(deps).log('[MODEL_DEBUG] forecast', debugData);
    }
}

function logWeeklyCorrectionDebug(debugData, deps: CalculationsDependencies = {}) {
    if (isDebugEnabled(deps)) {
        getLogger(deps).log('[MODEL_DEBUG] weekly correction', debugData);
    }
}

function calculateAge(birth_date, deps: CalculationsDependencies = {}) {
    if (!birth_date) {
        return null;
    }
    ...
    if (birth_date instanceof Date) {
        ...
        let normalized = raw;
        if (typeof deps.normalizeLocalDate === 'function') {
            // Сначала пробуем штатный нормализатор даты приложения.
            normalized = deps.normalizeLocalDate(raw) || raw;
        }
...
function adjustCaloriesByWeeklyProgress(profile, weeklyAverageWeight, deps: CalculationsDependencies = {}) {
    const debugWeeklyCorrection = (debugData) => logWeeklyCorrectionDebug(debugData, deps);
...
function calculateWeightGoalForecast(
    { sex, goal, tdee_calories, weight_kg, target_weight_kg, goal_deadline, now_date = null },
    deps: CalculationsDependencies = {}
) {
    const debugForecast = (debugData) => logForecastDebug(debugData, deps);
    const debugInput = { sex, goal, tdee_calories, weight_kg, target_weight_kg, goal_deadline };
    const logForecastPipeline = (stage, payload = {}) => {
        if (isDebugEnabled(deps)) {
            getLogger(deps).log('[MODEL_DEBUG] forecast:pipeline', { stage, ...payload });
        }
    };
...
function computeTargets(profile, nowDate = null, deps: CalculationsDependencies = {}) {
    ...
    const age = calculateAge(safeProfile?.birth_date, deps);
