...
function renderDiaryList(entries) {
    ...
    visibleEntries.forEach((entry) => {
        ...
        existing.water_l += Number(totals.water_l) || 0;
...
function hasEntryData(entry) {
    ...
    if (entry.mode === MODE_PRODUCTS) {
        ...
        const calories = Number(entry?.totals?.calories ?? entry?.calories);
    ...
    const fat = Number(entry?.fat_g);
    const carbs = Number(entry?.carbs_g);
    const water = Number(entry?.water_l);
    return (
...
function renderDailySummary(entries, date) {
    ...
    const totals = dayEntries.reduce(
        (acc, entry) => {
            ...
            acc.water_l += Number(entry.water_l) || 0;
            return acc;
        },
    ...
    container.innerHTML = `
        ...
        <div>Вода: ${totals.water_l.toFixed(1)} л</div>
    `;
}
