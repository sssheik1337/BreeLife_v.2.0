...
function renderWeeklyProgress() {
    ...
    for (let i = 0; i < 7; i += 1) {
        ...
        if (dayCalories > maxDayCalories) {
            maxDayCalories = dayCalories;
        }
        weekDays.push({
            dateKey,
            dateLabel,
            dayCalories,
            hasData,
            isToday: todayKey && dateKey === todayKey,
            label: dayLabels[i],
        });
    }

    const targetBase = Number.isFinite(targetCalories) && targetCalories > 0
        ? targetCalories
        : maxDayCalories;

    let totalPercent = 0;
    weekDays.forEach((day) => {
        const dayPercent = Number.isFinite(targetBase) && targetBase > 0
            ? Math.min(Math.max(day.dayCalories / targetBase, 0), 1)
            : 0;
        ...
        if (day.dateKey) {
            item.href = `/diary?date=${day.dateKey}&mode=summary`;
        } else {
        ...
        dateText.textContent = day.dateLabel;
