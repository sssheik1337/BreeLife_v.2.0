...
function getTodayDiaryTotals() {
    ...
    const totals = entries.reduce(
        (acc, entry) => {
            ...
            acc.water_l += resolved.water_l;
            return acc;
        },
        { calories: 0, protein_g: 0, fat_g: 0, carbs_g: 0, water_l: 0 }
    );

    const hasEntries = totals.calories > 0 || totals.protein_g > 0 || totals.fat_g > 0 || totals.carbs_g > 0 || totals.water_l > 0;
...
function renderWeeklyProgress() {
    ...
    entries.forEach((entry) => {
        ...
        const water = Number(totals.water_l) || 0;
        const current = waterByDate.get(normalizedDate) || 0;
        waterByDate.set(normalizedDate, current + water);
    });
    ...
    for (let i = 0; i < 7; i += 1) {
        ...
        const hasData = dateKey ? waterByDate.has(dateKey) : false;
        const dayPercent = Number.isFinite(waterTarget) && waterTarget > 0
            ? Math.min(Math.max(dayWater / waterTarget, 0), 1)
            : 0;
    ...
    descElement.textContent = 'Учитываются записи воды за неделю.';
}
